...
from qrokkun_ai.v2.obs import OBS_DIM, vectorize
from qrokkun_env.policies import FleeNearestBullet
from qrokkun_ai.v2.agents.player_v2 import PlayerV2
from qrokkun_ai.v2.agents.spawner_v2 import (
    SPAWNER_ACTIONS,
    SPAWNER_OBS_DIM,
    SpawnerV2,
    decode_spawner,
...
def act_player_net(net, env, device, sample: bool, temp: float):
    ...
    return int(a.item()), float(dist.log_prob(a).item()), float(value.item()), vectorize(env)
...
def main() -> None:
    ...
    ap.add_argument("--init-player", type=Path, default=Path("runs/both_player.pt"))
    ap.add_argument("--baseline-player", type=Path, default=Path("runs/baseline_v1/player_gpu.pt"))
    ap.add_argument("--baseline-spawner-old", type=Path, default=Path("runs/baseline_v1/spawner_gpu.pt"))
    ap.add_argument("--pool-player", type=Path, default=Path("runs/both_v1/player.pt"))
    ap.add_argument("--out-player", type=Path, default=Path("runs/both_v2_player.pt"))
    ap.add_argument("--out-spawner", type=Path, default=Path("runs/both_v2_spawner.pt"))
    ap.add_argument("--status", type=Path, default=Path("runs/both_v2_status.json"))
    ap.add_argument("--compare", type=Path, default=Path("runs/both_v2_compare.json"))
    ap.add_argument("--log", type=Path, default=Path("runs/both_v2.jsonl"))
    ...
    player = load_player(args.init_player, device)
    # New action space → fresh spawner (kind control); cannot load old 288-way head.
    spawner = SpawnerAC(hidden=args.hidden, n_actions=SPAWNER_ACTIONS).to(device)

    pool_players = []
    if args.baseline_player.exists():
        pool_players.append(("baseP", freeze(load_player(args.baseline_player, device))))
    if args.pool_player.exists():
        pool_players.append(("bothV1P", freeze(load_player(args.pool_player, device))))
    pool_players.append(("flee", "flee"))

    # Old 288-way spawner only for eval reference via separate loader if needed — skip as opponent
    # (incompatible action space). Use scripted + self + fresh learning.
    ...
    print(f"device={device} spawner_actions={SPAWNER_ACTIONS} flee|scripted={flee_vs_scripted:.2f}s", flush=True)
    ...
    cmp = {
        ...
        "updates": update,
        "spawner_actions": SPAWNER_ACTIONS,
        "notes": "v2: kind control, env jitter, opponent pool, no early-kill penalty",
    }
    torch.save({"state_dict": player.state_dict(), "hidden": args.hidden, "algo": "both-v2-player",
