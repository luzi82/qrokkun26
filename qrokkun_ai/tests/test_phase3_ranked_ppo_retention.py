...
* Input is a STRICT ``--init-checkpoint``: it must be a production
  ...
  ``qrokkun_ai.agents.player_checkpoints`` (legacy PlayerV1/PlayerV4 dict
...
* Two paired arms start from the exact same loaded initial state:
  ...
  scripted-Player PPO algorithm from ``qrokkun_ai.train.player_v1``, reused
...
from qrokkun_ai.train import player_v1 as scripted_ppo  # noqa: E402
...
def test_ppo_hyperparameters_match_current_scripted_player_ppo() -> None:
    ...
    defaults (qrokkun_ai/train/player_v1.py), never reinvented."""
