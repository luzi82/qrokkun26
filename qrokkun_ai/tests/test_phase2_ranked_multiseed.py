...
* Collect the canonical V1 teacher dataset exactly ONCE (data seed 4; 240
  ...
  ``TRAIN_SEEDS = (4, 5, 6)``.
* Model: ``qrokkun_ai.agents.player_ranked_topk.PlayerRankedTopK`` with
...
* Checkpoints are packed/validated through the STRICT production
  ``qrokkun_ai.agents.player_checkpoints`` provenance path (never the
...
from qrokkun_ai.agents.obs_v4 import (  # noqa: E402
    BULLET_FEAT_V4,
    MAX_BULLETS_V4,
    PAD_RADIUS,
    PLAYER_FEAT_V4,
...
def test_train_one_seed_uses_capacity_controls_selection_rule() -> None:
    from qrokkun_ai.tools.phase2_capacity_controls import should_replace_selected
