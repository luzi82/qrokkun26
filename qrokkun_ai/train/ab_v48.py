...
``qrokkun_ai/train/both_v4_args.py``) and does **not** modify PPO, rewards,
...
``qrokkun_ai.train.both_v4`` and persists a machine-readable manifest next
to each arm's own run outputs.

This module intentionally only touches ``argparse``/``json``/``subprocess``
(via ``qrokkun_ai.train.both_v4_args.build_parser``, which itself does not
...
from qrokkun_ai.train.both_v4_args import build_parser
...
def build_paired_configs(
    *,
    ...
    ``qrokkun_ai.train.both_v4``: identical seed/hours/training knobs
...
def build_ab_meta_parser() -> argparse.ArgumentParser:
    ap = argparse.ArgumentParser(
        ...
        description=(
            ...
            "qrokkun_ai.train.both_v4, writes a manifest for each arm, and "
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    for arm, fraction, manifest_path in (
        ...
        cmd = ["python", "-m", "qrokkun_ai.train.both_v4"] + argv_list
