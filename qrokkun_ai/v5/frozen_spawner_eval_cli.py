...
:mod:`qrokkun_ai.v5.frozen_spawner_seed_package` (immutable manifest) and
:mod:`qrokkun_ai.v5.frozen_spawner_eval_runner` (pure statistics) deliberately
...
* fail-closed, architecture-dispatched checkpoint loading
  ...
  checkpoint via :mod:`qrokkun_ai.v5.agents.player_checkpoints`), and the
...
* a single-episode runner (:func:`run_frozen_spawner_episode`) that evaluates
  ...
  ``qrokkun_ai.v4.train.both_v4.run_episode`` cannot do, since it ties both
...
* a CRN wrapper (:func:`run_frozen_spawner_episode_crn`) that forks the Torch
  ...
  mirroring ``qrokkun_ai.v4.eval_modes.run_eval_episode``'s reproducibility
  contract;
...
from qrokkun_ai.v5.agents.player_ranked_topk import ARCHITECTURE as RANKED_TOP_K_ARCHITECTURE
from qrokkun_ai.v1.agents.player_v1 import PlayerV1
from qrokkun_ai.v4.agents.spawner_v4 import SpawnerV4, spawn_continuous
from qrokkun_env.env import Qrokkun26Env, _spawn_interval
from qrokkun_ai.v4.eval_modes import MODE_DET_DET, MODE_DET_STOCH, MODE_STOCH_STOCH
from qrokkun_ai.v4.train.both_v4 import apply_player_action as _canonical_apply_player_action
from qrokkun_ai.v5.frozen_spawner_eval_runner import (
...
from qrokkun_ai.v1.obs import vectorize as vectorize_v1
...
# from (and identical to) qrokkun_ai.v5.frozen_spawner_seed_package's own
...
def load_player_v1_checkpoint(path: Path | str, device: torch.device | str = "cpu") -> tuple[PlayerV1, dict]:
    ...
    flat 45-dim observation (:func:`qrokkun_ai.v1.obs.vectorize`). No
...
def load_player_ranked_topk_checkpoint(
    ...
    delegates to :func:`qrokkun_ai.v5.agents.player_checkpoints.load_ranked_top_k_checkpoint`,
...
def apply_player_action(env: Qrokkun26Env, a: int) -> None:
    ...
    ``qrokkun_ai.v4.train.both_v4.apply_player_action``, instead of keeping a
...
def run_frozen_spawner_episode(
    ...
    *,
    ...
    Mirrors the physics loop of ``qrokkun_ai.v4.train.both_v4.run_episode``'s
...
def run_frozen_spawner_episode_crn(
    ...
    *,
    ...
    mirroring ``qrokkun_ai.v4.eval_modes.run_eval_episode``. This is what makes
...
# `qrokkun_ai.v4.eval_modes.PAIRED_EVAL_SEEDS` (3000..3029, 30 seeds) and the
