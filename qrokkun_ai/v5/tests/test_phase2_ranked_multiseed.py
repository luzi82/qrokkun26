...
* Collect the canonical V1 teacher dataset exactly ONCE (data seed 4; 240
  ...
  ``TRAIN_SEEDS = (4, 5, 6)``.
* Model: ``qrokkun_ai.v5.agents.player_ranked_topk.PlayerRankedTopK`` with
...
* Checkpoints are packed/validated through the STRICT production
  ``qrokkun_ai.v5.agents.player_checkpoints`` provenance path (never the
...
def test_train_one_seed_uses_capacity_controls_selection_rule() -> None:
    from qrokkun_ai.v5.tools.phase2_capacity_controls import should_replace_selected
